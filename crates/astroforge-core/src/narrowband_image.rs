//! Pre-existing narrowband image-processing helpers,
//! preserved verbatim. Originally lived in
//! `crates/astroforge-core/src/narrowband.rs`; renamed to
//! `narrowband_image` during CR-04 P7 to make room for
//! the §11 channel-detection + composition-suggestion
//! engine in `narrowband.rs`.
//!
//! This module owns image-side narrowband processing
//! helpers (`extract_channel`, `extract_oiii`,
//! `compose_palette`, `scnr_green`, `scnr_magenta`,
//! `normalize_channel_ratio`, `is_narrowband_session`).
//! The detection-side helpers (`from_filter_name`,
//! `detect_narrowband`, `primary_channel`, the
//! `NarrowbandFilter` enum) overlap with `narrowband` but
//! are kept here for callers that still depend on the
//! pre-P7 API.
//!
//! **Migration path:** the new `narrowband` module's
//! `NarrowbandChannel` enum supersedes `NarrowbandFilter`,
//! and `narrowband::detect_narrowband` supersedes the
//! here-detector. Callers should migrate to the new API;
//! the old symbols are kept here as a transition aid.
//!
//! These helpers are candidates for re-homing in
//! CR-14 (Narrowband Studio) — they belong with the
//! image-processing pipeline rather than the import
