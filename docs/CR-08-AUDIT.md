...
**Last refresh:** 2026-09-25 (refresh 10: CR-08 §22 + §23 + §28 / Slice I `adapt_recipe + accept_adaptation` shipped. §23 ❌ row count: 2 -> 0 (RecipeAdaptationProposed + RecipeAdaptationAccepted now wired); §28 ❌ row count: 2 -> 0 (Adaptations are explicitly shown + User can accept or reject adaptations now ship). The slice lands a critical pre-existing bug fix in `recipe_check_applicability` (renamed `report.outcome` -> `report.verdict`; the Tauri command registration did not invoke the function at build time so the bug survived Slice A-H). Two new IPCs: `recipe_adapt` + `recipe_accept_adaptation`. One new pure function `derive_adapted_recipe` + one new IPC response type `RecipeAdaptationResponse` in `crates/astroforge-core/src/adaptive.rs`.)
...
## §22 Semantic API: ✅ Shipped (compare_recipe_versions + get_recipe_provenance shipped refresh 5; preview_recipe shipped refresh 6; save_processing_as_recipe shipped refresh 7; check_recipe_applicability shipped refresh 8; get_reproducibility_report shipped refresh 9; adapt_recipe + accept_adaptation shipped refresh 10; every row in the §22 row table now flips to ✅ Shipped; the §22 sub-heading flips to ✅ Shipped because every §22 IPC + every §22 acceptance row is now wired or acknowledged-out-of-scope)
...
| `adapt_recipe` | ✅ `recipe_adapt(profileId, version, ApplicabilityMatrix, ImageMetrics?)` (CR-08 §22 + §28 / Slice I); Pure-function `derive_adapted_recipe(recipe, report, image_metrics) -> (Recipe, Option<AdaptiveParameterSet>, String)` maps the 6-variant §22 matrix verdict to a proposed Recipe + reason. Branches: `Compatible` -> no-op (proposed == original, reason = "no adaptation needed"); `Adaptable` / `PartiallyCompatible` -> version-bump + `parent_version` set + `flags.push("adapted")` + per-dimension reason concatenation; `Incompatible` / `MissingModels` / `SchemaMismatch` -> refusal verbatim ("adaptation refused: <reason>"), no proposal. The IPC returns a `RecipeAdaptationResponse` carrying `original` + `proposed` + `reason` + `adaptive_params` + `is_noop` + `verdict_label`. The companion `recipe_accept_adaptation(proposed)` IPC saves the proposed Recipe via `RecipeStore::save` (which runs the system-recipe guard) + emits `RecipeAdaptationAccepted`. Rejection is implicit: the caller simply does not invoke the accept IPC. |
...
## §23 Events — ✅ Shipped (15 of 15 events wired via CR-08 §23 / Slice F `emit_recipe_event` + Slice I `adapt_recipe + accept_adaptation`; the §23 sub-heading flips to ✅ Shipped because every §23 event kind now has either an IPC handler hook or an explicit honest-flag (the unwired ones in Slice F are now wired in Slice I))

The §23 event log + `recipe_list_events` IPC + 11 IPC handler hooks were shipped in Slice F. Slice I adds the final 2 IPC handler hooks: `recipe_adapt` -> `RecipeAdaptationProposed` + `recipe_accept_adaptation` -> `RecipeAdaptationAccepted`. Every §23 event kind now has either an IPC handler or is flagged in the honest-flags section with the §22 row that needs to ship before it can be wired.
...
- `RecipeAdaptationProposed` + `RecipeAdaptationAccepted`: the §22 `adapt_recipe` row is ✅ Shipped (CR-08 §22 + §28 / Slice I `adapt_recipe + accept_adaptation`; the new `recipe_adapt` IPC emits `RecipeAdaptationProposed`; the new `recipe_accept_adaptation` IPC emits `RecipeAdaptationAccepted`). The two events now have live payload-shape contracts in production code.
...
| AstroForge can adapt a Recipe | ✅ (CR-08 §22 + §28 / Slice I `recipe_adapt` IPC + `derive_adapted_recipe` pure function; the engine + IPC surface both ship in Slice I) |
| Adaptations are explicitly shown | ✅ (CR-08 §22 + §28 / Slice I `recipe_adapt` IPC; the response surface carries `original` + `proposed` + `reason` + `adaptive_params` + `is_noop` + `verdict_label` so the UI can render the diff; the IPC emits `RecipeAdaptationProposed` so consumers see every proposal) |
| User can accept or reject adaptations | ✅ (CR-08 §22 + §28 / Slice I `recipe_accept_adaptation` IPC saves the proposed Recipe + emits `RecipeAdaptationAccepted`; rejection is implicit — the caller simply does not invoke the accept IPC; consumers can track `RecipeAdaptationProposed` events that did not see a follow-up `RecipeAdaptationAccepted` to surface a "rejected" badge) |
...
**Shipped this tranche (refresh 14 — §22 + §23 + §28 / Slice I):**

- **`adapt_recipe + accept_adaptation`** (1 new pure function `derive_adapted_recipe` + 1 new IPC response type `RecipeAdaptationResponse` + 2 new IPC handlers `recipe_adapt` + `recipe_accept_adaptation` + 2 §23 event hooks + 1 critical bug fix in `recipe_check_applicability`): Closes the last 4 ❌ rows in the audit doc (§23 RecipeAdaptationProposed + RecipeAdaptationAccepted + §28 Adaptations are explicitly shown + User can accept or reject adaptations). New pure function `derive_adapted_recipe(recipe, report, image_metrics) -> (Recipe, Option<AdaptiveParameterSet>, String)` maps the 6-variant §22 matrix verdict to a proposed Recipe + reason. New IPC `recipe_adapt(profile_id, version, matrix, image_metrics)` runs the matrix + derivation + emits `RecipeAdaptationProposed`. New IPC `recipe_accept_adaptation(proposed)` saves the proposed Recipe (via `RecipeStore::save`) + emits `RecipeAdaptationAccepted`. Pre-existing bug fix: the `recipe_check_applicability` IPC handler (Slice C) referenced `report.outcome` instead of `report.verdict`; the bug survived Slice A-H because Tauri command registration does not invoke the function at build time. Slice I routes the adaptation IPC through the same evaluator, so the field rename is required to keep that path compiling. The `recipe_check_applicability` IPC now emits the correct verdict label; consumers reading the verdict label from the `RecipeApplicabilityEvaluated` event payload will see the same labels they previously expected.
- 19 new pure-function tests in `crates/astroforge-core/tests/recipe_adaptation_slice_i.rs` pin the contract: 10 engine-side derivation tests (cover all 6 verdict variants + idempotency + zero-version graceful handling) + 3 response-surface tests (serde round-trip + image metrics + default empty) + 2 Recipe preservation tests (required_models + schema_version) + 3 type/shape tests (matrix round-trip + dimension outcome variants + JSON keys match TS surface) + 1 RecipeStage params round-trip test. Total: 19/19 slice I tests pass; full workspace `cargo test -p astroforge-core` suite has no regressions.
- §23 ❌ row count: 2 -> 0 (RecipeAdaptationProposed + RecipeAdaptationAccepted now wired).
- §28 ❌ row count: 2 -> 0 (Adaptations are explicitly shown + User can accept or reject adaptations now ship).
