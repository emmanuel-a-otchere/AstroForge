...
**Version:** 1.3.0 (consolidated carrier)
...
**Carrier note:** This document is a carrier of `AstroForge_Spec_v1.1.0.md`
preserved in full. The 1.2.0 intermediate bump was a delta commit (CR-06
tranche) and did not author a standalone spec file; see `SPEC_INDEX.md`
§ Historical Specifications. The 1.3.0 carrier therefore reproduces the
1.1.0 base content below (sections 1–17) unchanged, then appends the
**Delta from 1.2.0** section that captures the substantive changes that
landed between 1.1.0 and 1.3.0. Where the delta changes a section, the
delta section links to the canonical CR document rather than re-authoring
prose.
...
This carrier captures the changes that landed in 1.3.0 (PRs #307–#311,
2026-09-12). The substantive content lives in the canonical CR documents
and the M9 audit; the carrier references them rather than re-authoring
prose.

## AI Enhancement Studio — Real ONNX inference (CR-06 P5.1, PR #307)

The P5.1 tranche replaces the P4 metadata-only dispatcher with a real
ONNX Runtime engine running bundled classical-kernel ONNX graphs. The
dispatcher contract (§7 stages 13–15) is unchanged; the engine inside
each AI stage is now real. Concretely:

- Five bundled classical-kernel ONNX graphs (`blur-blend`,
  `sharpen-blend`, `upscale-2x`, `hotpixel`, `masked-fill`) shipped
  via `include_bytes!` + SHA-256 pinning.
- Tile execution with `OES_texture_float` Float32 RGBA textures; mask
  subregion threading per tile.
- New `ai_quality_reports` table (SQLite migration v9) persists every
  gate verdict. The §37 `SegmentationLeakage` gate now compares real
  inside / outside region deltas instead of returning no-op.

**Canonical:** [../CR-06-AI-ENHANCEMENT-STUDIO.md](../CR-06-AI-ENHANCEMENT-STUDIO.md),
[../plans/2026-09-12-cr06-p5-1-real-onnx/PLAN.md](../plans/2026-09-12-cr06-p5-1-real-onnx/PLAN.md),
[M9 § P5.1 forward-look (closed)](../M9_AUDIT.md#p51-forward-look-closed-2026-09-12).

## Zone B Canvas + Compare Surfaces (CR-07, PRs #308 + #309 + #310)

The CR-07 programme ships the visual surface that consumes the AI
studio's output (§10.3 model registry + §11.4 safety + integrity badge).

- `read_image_artifact` Tauri command returns base64 16-bit TIFF bytes
  for any `ImageVersion::primary_artifact_id`.
- `ImageCanvas.svelte` component — pure rendering with zoom/pan, mask
  overlay, 256-bin histogram per channel, clipping overlay, region
  inspection.
- `CompareTools.svelte` — split-slider (vertical `clip-path` overlay),
  blink comparator, difference-map (`ImageData` per-pixel absolute
  delta), side-by-side.
- `image-canvas-webgl.ts` — self-contained WebGL 1 renderer with
  Float32 RGBA upload + Uint8 fallback for > 4 MP renders. Sticks to
  Canvas 2D on context-creation failure.

**Canonical:** [../CR-07-ZONE-B-CANVAS.md](../CR-07-ZONE-B-CANVAS.md),
[../plans/2026-09-12-cr07-zone-b-canvas/PLAN.md](../plans/2026-09-12-cr07-zone-b-canvas/PLAN.md).

## Forward-Look Bundle — DP#4, SessionCache, ADRs (PR #311)

Closes the forward-look items from M9 § P5.2 / CR-07 follow-ups + the
Open Decision Points table in PROJECT_PLAN.

- **DP#4 catalog license verification.** `OnnxEngine::open_catalog(id, bytes)`
  is digest-pinned against `CATALOG_MODELS`; unknown id fails closed,
  unpinned entry fails closed with `CatalogUnpinned`, mismatch fails
  closed with `ChecksumMismatch`. Real-catalog entries (SwinIR-denoise,
  SwinIR-sr, Swin2SR, star-seg, cloud-score, color-cal, trail-lama)
  carry `UNVERIFIED_SHA256` sentinel pending license + hash publication.
- **`SessionCache`.** Process-wide `Arc<Mutex<HashMap>>` cache keyed
  by `(kind, sha256)`. `get_or_build` runs the build closure outside
  the cache lock; `clear()` exposed for model-registry changes.
- **ADRs.** `docs/adr/0001-plate-solve-dependency.md` (closes #73:
  adopt ASTAP, bundled, offline) + `docs/adr/0002-smart-telescope-sdk.md`
  (closes #133: file-only ingest v1.x, SDK plugin Phase 4).

**Canonical:** [`docs/adr/README.md`](../adr/README.md),
[M9 § P5.1 forward-look (closed, 2026-09-12) — P5.2 / CR-07 follow-ups](../M9_AUDIT.md#p51-forward-look-closed-2026-09-12),
[PROJECT_PLAN § Open Decision Points](../PROJECT_PLAN.md#open-decision-points).

## M9 Audit (2026-09-12)

The M9 audit reconciles every criterion in §1–§17 of this spec against
the shipped code. **39 of 39 criteria ship**; zero remain partial or
missing.

**Canonical:** [../M9_AUDIT.md](../M9_AUDIT.md).

## Versioning

This 1.3.0 carrier preserves the 1.1.0 base content and adds the
**Delta from 1.2.0** section above. Future spec bumps follow the same
