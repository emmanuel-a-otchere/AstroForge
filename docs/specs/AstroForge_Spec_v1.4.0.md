...
**Version:** 1.4.0 (consolidated carrier)
...
**Carrier note:** This document is a carrier of `AstroForge_Spec_v1.3.0.md`
preserved in full. The 1.3.0 intermediate carrier was the first formal
delta-document (CR-06 P5.1 + CR-07 + ADRs + DP#4); the 1.4.0 carrier
reproduces the 1.3.0 carrier content below (sections 1–17 + 1.3.0 delta)
unchanged, then appends the **Delta from 1.3.0** section that captures
the substantive changes that landed between 1.3.0 and 1.4.0. Where the
delta changes a section, the delta section links to the canonical CR
document rather than re-authoring prose.
...
This carrier captures the changes that landed in 1.4.0 (PR #311,
2026-09-12 — forward-look bundle). The 1.4.0 cut is *additive* over
1.3.0; nothing in the 1.3.0 contract was removed or weakened. The
substantive content lives in the canonical CR documents and the M9
audit; the carrier references them rather than re-authoring prose.

## DP#4 catalog license verification (§10.8)

The §10.8 "Model Supply-Chain Integrity" section is now backed by
runtime machinery:

- `OnnxEngine::open_catalog(id, bytes)` looks up `CATALOG_MODELS` and
  verifies the runtime SHA-256 against the pinned value.
- Unknown id → `InferenceError::UnknownCatalogModel` (fail closed).
- Unpinned entry (`sha256 = "unverified"`) →
  `InferenceError::CatalogUnpinned` (fail closed until real hash lands).
- Mismatch → `InferenceError::ChecksumMismatch` (fail closed).
- Legacy `open_catalog_unpinned(bytes, kind)` preserved for tests +
  pre-DP#4 callers.

The 7 real-catalog entries from PROJECT_PLAN P2-M1-T5..T11
(SwinIR-denoise, SwinIR-sr, Swin2SR, star-seg, cloud-score, color-cal,
trail-lama) carry `UNVERIFIED_SHA256` sentinel pending license + hash
publication; closing each is a single-line table update.

## SessionCache (§10.7 model notes)

`SessionCache` (in `crates/astroforge-ai/src/inference.rs`) cuts
session-build cost from N (per-apply) to 1 (per-model lifetime) for
batch scenarios. The cache is process-wide, keyed by `(kind, sha256)`,
with `get_or_build(key, || …)` running the build closure outside the
cache lock so a slow build doesn't block concurrent reads.

## ADRs (§17 open items #8 and #1)

Two open items from §17 are now closed via ADR rather than resolved
in code:

- **#8 (plate-solve dependency):** [ADR-0001](../adr/0001-plate-solve-dependency.md)
  decides ASTAP bundled + offline. astrometry.net deferred (offline-first
  spec constraint).
- **#1 (smart-telescope SDK):** [ADR-0002](../adr/0002-smart-telescope-sdk.md)
  decides file-only ingest is the v1.x contract; SDK integration
  deferred to Phase 4 plugin (per ADR-0002 § Decision).

## M9 Audit forward-look walk-down

The M9 forward-look items are now:

| Item | Status |
|---|---|
| DP#4 fail-closed machinery | ✅ Shipped (this release) |
| SessionCache for apply-round session reuse | ✅ Shipped (this release) |
| ADR #73 (plate-solve) | ✅ Decided (ADR-0001) |
| ADR #133 (smart-telescope SDK) | ✅ Decided (ADR-0002) |
| Real-catalog digest pinning (7 entries) | 🟡 Pending upstream license + hash |
| GPU execution providers (CUDA / DirectML / Metal) | 🟡 Pending per-platform CI matrix |
| Spec carrier authoring (1.3.0, 1.4.0) | ✅ Shipped (this + the previous release) |

**Canonical:** [M9 § P5.1 forward-look (closed, 2026-09-12)](../M9_AUDIT.md#p51-forward-look-closed-2026-09-12).

## Versioning

This 1.4.0 carrier preserves the 1.3.0 carrier content and adds the
**Delta from 1.3.0** section above. Future spec bumps follow the same
